...
def save_pic(pic_src, pic_cnt):
    """
    将图片下载到本地文件夹
    pic_src 图片下载地址 来自urls_crawler
    pic_cnt 专辑号码 来自urls_crawler
    """
    try:
        img = requests.get(pic_src, headers=HEADERS, timeout=10)
        img_name = "pic_cnt_{}.jpg".format(pic_cnt + 1) # 加一从一开始 第一个pic_cnt是0
        with open(img_name, 'ab') as f: # 打开文件名为img_name
...
def make_dir(folder_name):
    ...
    if not os.path.exists(path):
        os.makedirs(path)
        print(path)
        os.chdir(path) # 切换到的新路径  如果允许访问返回 True, 否则返回False
        return True
...
def delete_empty_dir(save_dir):
    ...
    save_dir 来自main 的DIR_PATH 和 本函数的path
    """
    if os.path.exists(save_dir):
        if os.path.isdir(save_dir):
            for d in os.listdir(save_dir): # listdir把目录下文件列出来
                path = os.path.join(save_dir, d)     # 组装下一级地址
                if os.path.isdir(path): # 如果组装后的地址还是目录的话, 在丢入本函数检查
                    delete_empty_dir(path)      # 递归删除空文件夹
        if not os.listdir(save_dir): # 如果目录下不存在文件或文件夹
            os.rmdir(save_dir) # 删除本目录
        ...
        with lock:
            if make_dir(folder_name):
                ...
                max_count = BeautifulSoup(r, 'lxml').find(
                    ...
                    #获取class=page 的倒数第二个a标签的文本内容 即总页数
                # 套图页面
                ...
                for index, page_url in enumerate(page_urls):
                    ...
                    if index + 1 < len(page_urls):
                        ...
                        img_urls.append(img_url)
                        # BeautifulSoup.find哪一个标签再哪一个标签 只要哪部分内容用[]
                        # 这个判断没用了, 最后一张图也是a标签下的 打开是上一张专辑
